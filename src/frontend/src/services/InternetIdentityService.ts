...
export class InternetIdentityService {
  ...
  // 添加借贷位置
  async addBorrowPosition(asset: string, amount: number, rate: number): Promise<BorrowPosition> {
    if (!this.actor) {
      throw new Error('Actor未初始化');
    }
  ...
  // 添加收益位置
  async addEarnPosition(asset: string, amount: number, apy: number): Promise<EarnPosition> {
    if (!this.actor) {
      throw new Error('Actor未初始化');
    }
