...
export const TokenBalanceExample: React.FC = () => {
  ...
  const queryCustomTokenBalance = async () => {
    ...
    try {
      ...
      // 查询余额
      const balance = await internetIdentityService.queryCurrentUserBalance(customCanisterId);
      
      // 获取代币信息
      const tokenInfo = await internetIdentityService.getTokenInfo(customCanisterId);
  ...
  return (
    <div className="rounded-lg bg-white p-6 shadow-md dark:bg-gray-800">
      ...
      <div className="mt-4 text-xs text-gray-500 dark:text-gray-400">
        <p><strong>{t('supported_tokens')}:</strong></p>
