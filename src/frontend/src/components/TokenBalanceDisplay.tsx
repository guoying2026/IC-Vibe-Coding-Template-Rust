...
export const TokenBalanceDisplay: React.FC<TokenBalanceDisplayProps> = ({ principal }) => {
  ...
  return (
    <div className="token-balance-display">
        ...
        {Object.entries(balances).map(([tokenName, balance]) => (
          <div key={tokenName} className="balance-item">
            ...
            <button 
              onClick={() => {
                const tokenList = getTokenList();
                fetchSingleBalance(tokenList[tokenName as keyof typeof tokenList], tokenName);
              }}
      ...
      <div className="environment-info">
        <p>当前环境: {import.meta.env.VITE_DFX_NETWORK === "ic" ? "主网" : "本地开发"}</p>
        <p>Host: {import.meta.env.VITE_DFX_NETWORK !== "ic" ? "http://localhost:8080" : "https://ic0.app"}</p>
      </div>
    </div>
  );
}; 