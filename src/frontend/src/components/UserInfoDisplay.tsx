...
async function accountIdentifierFromPrincipal(principal: Principal, subaccount?: Uint8Array): Promise<string> {
  try {
...
export const UserInfoDisplay = ({
  ...
  const [copyFeedback, setCopyFeedback] = useState<{type: 'principal' | 'account' | null, message: string}>({type: null, message: ''});
  ...
  React.useEffect(() => {
    const generateAccountId = async () => {
      try {
        ...
        const id = await accountIdentifierFromPrincipal(Principal.fromText(principal.toText()));
        setAccountId(id);
      } catch (error) {
        console.error("生成Account ID失败:", error);
        setAccountId("生成失败");
  ...
  // 复制到剪贴板并显示反馈
  const copyToClipboard = async (text: string, type: 'principal' | 'account') => {
    try {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2 rounded-full bg-gradient-to-r from-green-50 to-emerald-50 px-4 py-2 dark:from-green-900/20 dark:to-emerald-900/20">
          <div className={`h-3 w-3 rounded-full ${isAuthenticated ? 'bg-green-500 shadow-lg shadow-green-500/50' : 'bg-gray-400'}`}></div>
          <span className={`text-sm font-medium ${isAuthenticated ? 'text-green-700 dark:text-green-400' : 'text-gray-500 dark:text-gray-400'}`}>
        ...
        <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-50 to-blue-50 p-6 shadow-lg transition-all duration-300 hover:shadow-xl dark:from-slate-800 dark:to-blue-900/20">
          ...
          <div className="relative">
            ...
            <div className="mb-3 flex items-center space-x-2">
              <div className="flex-1 rounded-lg bg-white/80 p-3 font-mono text-sm text-gray-900 dark:bg-gray-800/80 dark:text-white min-h-[44px] flex items-center">
                {isAuthenticated && principal ? (showPrincipal ? principal.toText() : mask(principal.toText())) : "-"}
              </div>
                ...
                <div className="flex space-x-1">
                  ...
                  >
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={showPrincipal ? "M13.875 18.825A10.05 10.05 0 0112 19c-5.523 0-10-4.477-10-10 0-1.657.403-3.221 1.125-4.575M15 12a3 3 0 11-6 0 3 3 0 016 0z" : "M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c2.042 0 3.97.613 5.542 1.675M19.542 12c-1.274 4.057-5.065 7-9.542 7-2.042 0-3.97-.613-5.542-1.675"} />
                    </svg>
                  </button>
                  <button 
                    onClick={() => copyToClipboard(principal.toText(), 'principal')} 
        ...
        <div className="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-50 to-purple-50 p-6 shadow-lg transition-all duration-300 hover:shadow-xl dark:from-slate-800 dark:to-purple-900/20">
          ...
          <div className="relative">
            ...
            <div className="mb-3 flex items-start space-x-2">
              <div className="flex-1 rounded-lg bg-white/80 p-3 font-mono text-sm text-gray-900 dark:bg-gray-800/80 dark:text-white break-all min-h-[44px] flex items-center">
                {isAuthenticated ? (showAccount ? accountId : mask(accountId)) : "-"}
              </div>
              {isAuthenticated && (
                <div className="flex space-x-1">
                  <button 
                  ...
                  >
                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={showAccount ? "M13.875 18.825A10.05 10.05 0 0112 19c-5.523 0-10-4.477-10-10 0-1.657.403-3.221 1.125-4.575M15 12a3 3 0 11-6 0 3 3 0 016 0z" : "M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c2.042 0 3.97.613 5.542 1.675M19.542 12c-1.274 4.057-5.065 7-9.542 7-2.042 0-3.97-.613-5.542-1.675"} />
                    </svg>
                  </button>
                  <button 
